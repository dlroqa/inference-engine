...
(``/v1/models``, ``/v1/chat/completions``, ``/v1/completions``) and adapts the
streamed SSE response onto the internal :class:`GenerationStream`. The rest of
the engine (edge, scheduler, serving lifecycle) is unchanged: a remote backend is
just another :class:`InferenceBackend`.

Scope for this sub-slice (see docs/backends.md):
...
class RemoteVLLMConfig:
    """Immutable connection/mapping settings for a remote vLLM backend."""
...
class RemoteVLLMBackend(InferenceBackend):
    ...
    def __init__(self, config: RemoteVLLMConfig) -> None:
        self._cfg = config
    ...
    def capabilities(self) -> Capabilities:
        ...
        return Capabilities(
            ...
            # vLLM supports guided decoding, but mapping it honestly is a later
            # sub-slice; report False so the edge rejects structured requests
    ...
    def _request_body(self, request: GenerationRequest) -> tuple[str, dict[str, Any]]:
        ...
        # vLLM accepts these OpenAI-superset sampler params in the request body.
        if request.top_k:
