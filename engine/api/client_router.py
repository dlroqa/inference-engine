...
def client_context(request: Request) -> ClientContext:
    """Authenticate a client by API key and resolve their client id.

    This is the isolation boundary: the client id comes from the key, never from
    the request, so a caller can only ever act as themselves.
    """
